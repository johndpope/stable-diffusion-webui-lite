...
import modules.prompt_helper.artists
import modules.interrogator.interrogate
from modules.utils.mem_monitor import MemUsageMonitor
import modules.diffuser.sd_models
import modules.prompt_helper.styles
import modules.utils as devices
from modules import sd_samplers
from modules.paths import MODEL_PATH, BASE_PATH, REPO_PATHS, RESOURCE_PATH
from modules.diffuser import hypernetwork

sd_model_file = os.path.join(BASE_PATH, 'model.ckpt')
...
# model arch & weight ckpt
parser.add_argument("--config", type=str, default=os.path.join(REPO_PATHS['stable-diffusion'], "configs/stable-diffusion/v1-inference.yaml"), help="path to config which constructs model")
...
parser.add_argument("--embeddings-dir", type=str, default=os.path.join(BASE_PATH, 'embeddings'), help="embeddings directory for textual inversion (default: embeddings)")
...
parser.add_argument("--ui-config-file", type=str, help="filename to use for ui configuration", default=os.path.join(BASE_PATH, 'ui-config.json'))
parser.add_argument("--ui-settings-file", type=str, help="filename to use for ui settings", default=os.path.join(BASE_PATH, 'config.json'))
parser.add_argument("--gradio-img2img-tool", type=str, help='gradio image uploader tool: can be either editor for ctopping, or color-sketch for drawing', choices=["color-sketch", "editor"], default="editor")
parser.add_argument("--opt-channelslast", action='store_true', help="change memory type for stable diffusion to channels last")
parser.add_argument("--styles-file", type=str, help="filename to use for styles", default=os.path.join(BASE_PATH, 'styles.csv'))
...
hypernetworks = hypernetwork.list_hypernetworks(os.path.join(MODEL_PATH, 'hypernetworks'))
...
state = State()
artist_db = modules.prompt_helper.artists.ArtistsDatabase(os.path.join(RESOURCE_PATH, 'artists.csv'))
prompt_styles = modules.prompt_helper.styles.StyleDatabase(cmd_opts.styles_file)
interrogator = modules.interrogator.interrogate.InterrogateModels("interrogate")
face_restorers = []


def realesrgan_models_names():
    import modules.upsampler.realesrgan_model
    return [x.name for x in modules.upsampler.realesrgan_model.get_realesrgan_models(None)]
...
mem_mon = MemUsageMonitor("MemMon", device, opts)
mem_mon.start()
